                ...
                <div x-show="open" x-collapse>
                    @foreach($projectNode['tickets'] as $ticketNode)
                        @php
                            ...
                            $ticketUrl = route('filament.resources.tickets.share', $ticket->code);
                        @endphp
                        <div x-data="{ expanded: false }" class="border-t border-gray-200 dark:border-gray-700">
                            {{-- Ticket header --}}
                            <button type="button" x-on:click="expanded = !expanded"
                                class="flex items-center justify-between w-full gap-4 px-5 py-3 pl-12 text-left transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                <div class="flex items-center gap-3 min-w-0">
                                    ...
                                    <span class="font-mono text-xs font-semibold text-primary-600 dark:text-primary-400 shrink-0">{{ $ticket->code }}</span>
                                ...
                                <div class="flex items-center gap-3 shrink-0">
                                    <div class="flex -space-x-1.5">
                                        @foreach($ticketNode['users']->take(4) as $member)
                                            ...
                                            @endphp
                                            <img src="{{ $memberAvatar }}" alt="{{ $member->name }}" title="{{ $member->name }}" class="w-6 h-6 rounded-full ring-2 ring-white dark:ring-gray-800" loading="lazy">
                                        @endforeach
                                        @if($ticketNode['users']->count() > 4)
                                            <span class="flex items-center justify-center w-6 h-6 text-[9px] font-semibold text-gray-500 bg-gray-200 rounded-full dark:bg-gray-600 dark:text-gray-300 ring-2 ring-white dark:ring-gray-800">+{{ $ticketNode['users']->count() - 4 }}</span>
                                        @endif
                                    </div>
                                    <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{{ $ticketNode['activities']->count() }} {{ __('changes') }}</span>
                                    <a href="{{ $ticketUrl }}" target="_blank" x-on:click.stop class="p-1.5 text-gray-400 rounded-md hover:text-primary-500 hover:bg-gray-100 dark:hover:bg-gray-700" title="{{ __('Open ticket') }}">
                            ...
                            {{-- Activity entries --}}
                            <div x-show="expanded" x-collapse class="bg-gray-50/50 dark:bg-gray-900/30">
                                @foreach($ticketNode['activities'] as $activity)
                                    @php
                                        ...
                                        $avatarUrl = $user->getAttributes()['avatar_url']
                                            ?? ('https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&size=64&background=' . substr(md5($user->id), 0, 6) . '&color=ffffff');
                                    @endphp
                                    <div class="flex items-center gap-3 py-2.5 pl-20 pr-5 border-t border-gray-100 dark:border-gray-700/40">
                                        ...
                                        <span class="text-xs font-semibold text-gray-700 dark:text-gray-300 shrink-0 min-w-[80px]">{{ $user->name }}</span>
                                        @if($oldStatus && $newStatus)
                                        <div class="flex items-center gap-1.5">
                                            <span class="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-medium rounded-md" style="background-color: {{ $oldStatus->color ?? '#6B7280' }}20; color: {{ $oldStatus->color ?? '#6B7280' }}">
                                                <span class="w-1.5 h-1.5 rounded-full" style="background-color: {{ $oldStatus->color ?? '#6B7280' }}"></span>
                                                {{ $oldStatus->name }}
                                            </span>
                                            ...
                                            <span class="inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-medium rounded-md" style="background-color: {{ $newStatus->color ?? '#6B7280' }}20; color: {{ $newStatus->color ?? '#6B7280' }}">
                                                <span class="w-1.5 h-1.5 rounded-full" style="background-color: {{ $newStatus->color ?? '#6B7280' }}"></span>
                                                {{ $newStatus->name }}
                                            </span>
                                        </div>
                                        @endif
                                        <span class="ml-auto text-xs text-gray-400 dark:text-gray-500 shrink-0">{{ $activity->created_at->format('M d, H:i') }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
