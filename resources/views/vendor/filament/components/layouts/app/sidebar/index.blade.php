    ...
    <header @class([
        ...
        >
            @if (config('filament.layout.sidebar.is_collapsible_on_desktop') && (config('filament.layout.sidebar.collapsed_width') !== 0))
                <button
                    type="button"
                    class="filament-sidebar-collapse-button shrink-0 hidden lg:flex items-center justify-center w-10 h-10 text-gray-100 dark:text-gray-200 rounded-full hover:bg-gray-500/10 focus:bg-gray-500/15 focus:outline-none transition-colors"
                    ...
                    x-transition:enter-end="opacity-100"
                >
                    {{-- Lucide PanelLeftClose — Notion / Linear style sidebar toggle --}}
                    <svg class="h-5 w-5" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        @if (config('filament.layout.sidebar.is_collapsible_on_desktop'))
            <button
                type="button"
                class="filament-sidebar-close-button shrink-0 flex items-center justify-center w-10 h-10 text-gray-100 dark:text-gray-200 rounded-full hover:bg-gray-500/10 focus:bg-gray-500/15 focus:outline-none transition-colors"
                x-bind:aria-label="$store.sidebar.isOpen ? '{{ __('filament::layout.buttons.sidebar.collapse.label') }}' : '{{ __('filament::layout.buttons.sidebar.expand.label') }}'"
                x-on:click.stop="$store.sidebar.isOpen ? $store.sidebar.close() : $store.sidebar.open()"
                ...
                x-transition:enter-end="opacity-100"
            >
                {{-- Lucide PanelLeftOpen — same panel shape, outward chevron when sidebar is closed --}}
                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round">
                    <rect x="3" y="3" width="18" height="18" rx="2"/>
                    <path d="M9 3v18"/>
